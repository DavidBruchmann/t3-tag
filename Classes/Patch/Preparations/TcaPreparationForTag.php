...
namespace WapplerSystems\Tag\Preparations;
...
class TcaPreparationForTag
{
    ...
    {
        return $this->configureTagRelations($tca);
    }

    /**
     * Prepares TCA configuration of type='tag' fields.
     *
     * It adds some TCA config settings so tag fields end up with similar
     ...
     *
     * Finally all tag fields with a "manyToMany" relationship are
     * added to the MM_oppositeUsage of sys_tag "items".
     *
     ...
     * @param array $tca
     */
    protected function configureTagRelations(array $tca): array
    {
        foreach ($tca as $table => &$tableDefinition) {
            ...
            foreach ($tableDefinition['columns'] as $fieldName => &$fieldConfig) {
                if (($fieldConfig['config']['type'] ?? '') !== 'tag') {
                    continue;
                }
                ...
                if (!in_array($fieldConfig['config']['relationship'], ['oneToOne', 'oneToMany', 'manyToMany'], true)) {
                    throw new \RuntimeException(
                        $fieldName . ' of table ' . $table . ' is defined as type tag with relationship "'
                ...
                if ($fieldConfig['config']['relationship'] === 'oneToOne') {
                    ...
                    if ((int)($fieldConfig['config']['maxitems'] ?? 0) > 1) {
                        throw new \RuntimeException(
                            $fieldName . ' of table ' . $table . ' is defined as type tag with an oneToOne relationship. ' .
                    ...
                    throw new \RuntimeException(
                        $fieldName . ' of table ' . $table . ' is defined as type tag with a ' . $fieldConfig['config']['relationship'] .
                ...
                // This will not be done for the sys_tag table itself.
                if ($fieldConfig['config']['relationship'] === 'manyToMany' && $table !== 'sys_tag') {
                    ...
                    // Register opposite references for the foreign side of a tag relation
                    if (empty($tca['sys_tag']['columns']['items']['config']['MM_oppositeUsage'][$table])) {
                        $tca['sys_tag']['columns']['items']['config']['MM_oppositeUsage'][$table] = [];
                    }
                    if (!in_array($fieldName, $tca['sys_tag']['columns']['items']['config']['MM_oppositeUsage'][$table], true)) {
                        $tca['sys_tag']['columns']['items']['config']['MM_oppositeUsage'][$table][] = $fieldName;
                    }
